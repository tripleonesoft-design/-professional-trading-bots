   ...
   input group "=== Trailing Stop (ETH Optimized) ==="
...
const int RSI_ZONE_OVERBOUGHT         = 58;
   ...
   Trade_Signal Analyze_Market() {
      ...
      if(Uptrend) {
         ...
         Result.Limit_Price = Entry_Price + Result.ATR_Value * 0.1;
         Result.Stop_Price = Entry_Price + Result.ATR_Value * 0.2;
   ...
   bool Is_Fair_Value_Gap(int Direction, int lookback = 6) {
      for(int bar = 0; bar <= lookback; bar++) {
