//+------------------------------------------------------------------+
//|                                     SmartConsensus_BTC.mq5      |
//|                        Bitcoin (BTCUSD) Optimized v1.0          |
//+------------------------------------------------------------------+
#property copyright "Professional Trading System"
#property version   "1.13"
#property description "Bitcoin Optimized - Signal Memory / Lookback Windows - Max Spread 3000, ATR 1000, SL 0.5x"
...
const double SL_MULTIPLIER = 0.5;
...
int OnInit() {
   ...
   Print("=======================================================");
   Print("   SmartConsensus BTC v1.0 - Bitcoin Optimized       ");
   Print("=======================================================");
   ...
   Print("SL Multiplier: ", SL_MULTIPLIER, "x ATR (safer for crypto)");
...
double Calculate_Lot_Size(double Entry_Price, double Stop_Loss, double ATR) {
   ...
   Lot = NormalizeDouble(MathMax(Lot, Min_Lot), 3);
...
bool Is_Volume_Spike_Recent(int lookback = 3) {
...
bool Is_Range_Expansion_Recent(int lookback = 3) {
    for(int bar = 0; bar <= lookback; bar++) {
...
bool Is_Liquidity_Swept(int Direction, int lookback = 6) {
    for(int bar = 0; bar <= lookback; bar++) {
...
bool Is_Consolidation_Breakout(int Direction, int lookback = 5) {
    for(int bar = 0; bar <= lookback; bar++) {
...
bool Is_Fair_Value_Gap(int Direction, int lookback = 8) {
    for(int bar = 0; bar <= lookback; bar++) {
